...
const Item = ({ data }) => {
  ...
  const handleClick = e => {
    ...
    e.target.id === "remove" &&
      removePickedItem(`${[data.parent2]}-${[data.parent1]}-${[data.id]}`);
  };
    ...
    <>
      {data && (
        ...
        >
          ...
          <Card.Header className="h5 pt-1">{data.author}</Card.Header>
          <Card.Body className="d-flex flex-column">
            <Card.Text>{data.name}</Card.Text>
            <div className="mt-auto d-flex justify-content-between align-items-center">
              <ItemModal data={data} />

              {pickedItems.includes(
                `${[data.parent2]}-${[data.parent1]}-${[data.id]}`
              ) ? (
                <img
                  id="remove"
                  onClick={handleClick}
